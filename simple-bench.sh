...
disk_latency_test() {
  ...
  for iter in $(seq 1 ${sample_count}); do
    ...
    if [ "hostx" == "${container_image}x" ]; then
      ...
      test_cmd="time -p dd if=/dev/zero of=${test_filename} bs=512 count=1000 oflag=dsync "
      ${prefix_cmd} ${test_cmd}
    else
...
disk_throughput_test() {
  ...
  for iter in $(seq 1 ${sample_count}); do
    ...
    if [ "hostx" == "${container_image}x" ]; then
      ...
      test_cmd="time -p dd if=/dev/zero of=${test_filename} bs=1G count=1 oflag=dsync"
      ${prefix_cmd} ${test_cmd}
    else
